var View = Backbone.View.extend({
  ...
  initialize: function () {
    ...
    this._socket.on(
      "config",
      _.bind(function (message, configs) {
  ...
  _onAppState: function (message) {
    ...
    $("#uptime").html(
      moment.duration(message.uptime, "milliseconds").format("dd:hh:mm:ss")
    );
    $("#downtime").html(
    ...
    $("#memory").html(
      message.memory && message.memory.length
        ? humanize.filesize(message.memory[message.memory.length - 1])
        : ""
    );
    ...
    $("#fps").html(
      message.fps && message.fps.length
        ? message.fps[message.fps.length - 1]
        : ""
    );
    ...
    $("#cpu").html(
      message.cpu && message.cpu.length
        ? message.cpu[message.cpu.length - 1].toFixed(2) + "%"
        : ""
    );
  ...
  _onConfig: function (message, configs) {
    ...
    $("#controls-configs").toggle(
      message.permissions.app && configs.length > 1
    );
    $("#controls-computer").toggle(message.permissions.computer);
    $("#controls").toggle(
      message.permissions.app || message.permissions.computer
    );
  },
  ...
  _onShutdownAppClicked: function () {
    if (
      window.confirm(
        "Are you sure you want to shut down the app? It will not restart automatically."
      )
    ) {
  ...
  _onRestartAppClicked: function () {
    if (
      window.confirm("Are you sure you want to shut down and restart the app?")
    ) {
  ...
  _onShutdownPcClicked: function () {
    if (
      window.confirm(
        "Are you sure you want to shut down the computer? It will not restart automatically."
      )
    ) {
  ...
  _onRestartPcClicked: function () {
    if (
      window.confirm(
        "Are you sure you want to shut down and restart the computer?"
      )
    ) {
  ...
  _onConfigClicked: function (event, config) {
    if (
      window.confirm(
        "Are you sure you want to shut down the app and launch " + config + " ?"
      )
    ) {
